...
export default function AdminLayout({ children }) {
  ...
  return (
    <div className={`h-screen w-screen ${theme === 'dark' ? 'bg-[#0A0F1A] text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      <div className="grid grid-cols-[270px_1fr] h-full">
        {/* Columna izquierda: sidebar fijo */}
        <Sidebar />

        {/* Columna derecha: contenido con scroll */}
        <main className={`h-full overflow-auto ${theme === 'dark' ? 'bg-[#0A0F1A]' : 'bg-slate-50'}`}>
