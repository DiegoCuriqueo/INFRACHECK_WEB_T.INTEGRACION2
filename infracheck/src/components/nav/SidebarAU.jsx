...
/* ----- NavItem optimizado (sin useLocation interno) ----- */
...
/* ----- Sidebar Autoridad (con guard anti-duplicado) ----- */
function SidebarAU() {
  const { pathname } = useLocation();

  // 🔒 Guard anti-duplicado: si ya hay un Sidebar AU montado, no renderizamos otro
  const alreadyMounted =
    typeof window !== "undefined" && window.__sidebars && window.__sidebars.AU;

  useEffect(() => {
    if (typeof window === "undefined") return;
    window.__sidebars = window.__sidebars || {};
    window.__sidebars.AU = true;
    return () => {
      window.__sidebars.AU = false;
    };
  }, []);
  ...
  const items = useMemo(
    () => [
      { to: "/autoridad/home", label: "Home", icon: Icon.home, exact: true },
      { to: "/autoridad/reportes", label: "Reportes", icon: Icon.report },
      { to: "/autoridad/perfil", label: "Perfil", icon: Icon.user },
      { to: "/autoridad/ajustes", label: "Ajustes", icon: Icon.settings },
    ],
    []
  );
  ...
  return (
    <aside
      role="navigation"
      aria-label="Sidebar autoridad"
      className="h-full w-[260px] bg-[#0B1220] bg-gradient-to-b from-[#0B1220] to-[#0A0F1A] border-r border-slate-800/80 px-4 py-5 relative"
      // Si tienes un drawer móvil aparte, cambia la línea de arriba por:
      // className="hidden md:block h-full w-[260px] bg-[#0B1220] bg-gradient-to-b from-[#0B1220] to-[#0A0F1A] border-r border-slate-800/80 px-4 py-5 relative"
    >
      {/* Header */}
      <div className="flex items-center gap-3 px-2 pb-6">
        ...
        <div>
          ...
          <div className="text-[11px] text-slate-400">Gestión de denuncias</div>
        </div>
      </div>
        ...
        <div className="px-3">
          <button className="w-full flex items-center gap-3 rounded-2xl bg-slate-800/40 hover:bg-slate-700/40 transition px-3 py-2.5 ring-1 ring-white/5">
            ...
            <div className="min-w-0 text-left">
              <p className="text-sm text-slate-100 leading-5">Autoridad</p>
              <p className="text-[11px] text-slate-400 -mt-0.5">Perfil activo</p>
            </div>
