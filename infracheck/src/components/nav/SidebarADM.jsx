...
/* ----- NavItem memoizado (sin useLocation) ----- */
const NavItem = memo(
  function NavItem({ to, icon, label, active, exact }) {
    return (
      <NavLink
        to={to}
        end={!!exact}
  ...
  // Comparador para evitar re-renders inútiles
  (prev, next) =>
    prev.to === next.to &&
...
/* ----- Sidebar Administrador (con guard anti-duplicado) ----- */
function SidebarADM() {
  const { pathname } = useLocation();

  // 🔒 Guard: si ya hay un Sidebar ADM montado, no renderizamos otro
  useEffect(() => {
    if (typeof window === "undefined") return;
    window.__sidebars = window.__sidebars || {};
    if (window.__sidebars.ADM) {
      // ya existe -> marcamos bandera para este ciclo y NO renderizamos
      // truco: guardamos una marca y forzamos un estado local si quisieras ocultar condicionalmente
    } else {
      window.__sidebars.ADM = true;
    }
    return () => {
      if (window.__sidebars) window.__sidebars.ADM = false;
    };
  }, []);

  if (typeof window !== "undefined" && window.__sidebars && window.__sidebars.ADM && document.querySelector('[aria-label="Sidebar administrador"]')) {
    // Ya hay uno en el DOM -> no pintamos otro
    return null;
  }

  const items = useMemo(
    () => [
      { to: "/admin/home", label: "Home", icon: Icon.home, exact: true },
      { to: "/admin/reportes", label: "Reportes", icon: Icon.report },
      { to: "/admin/perfil", label: "Perfil", icon: Icon.user },
  ...
  return (
    <aside
      role="navigation"
      aria-label="Sidebar administrador"
    ...
    >
      ...
      <div className="flex items-center gap-3 px-2 pb-6">
        ...
        <div>
          ...
          <div className="text-[11px] text-slate-400">Panel Administrador</div>
        </div>
      </div>
        ...
        <div className="px-3">
          <button className="w-full flex items-center gap-3 rounded-2xl bg-slate-800/40 hover:bg-slate-700/40 transition px-3 py-2.5 ring-1 ring-white/5">
            ...
            <div className="min-w-0 text-left">
              <p className="text-sm text-slate-100 leading-5">Administrador</p>
              <p className="text-[11px] text-slate-400 -mt-0.5">Cuenta principal</p>
            </div>
...
export default memo(SidebarADM);
